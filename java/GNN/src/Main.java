...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		for (int r = 0; r < R; ++r) {
			...
			for (int i = 0; i < M; ++i) {
				assert(!inSubset[subset[r][i]]);
				inSubset[subset[r][i]] = true;
			}
			
			
			float[][] Dt = new float[N][];
			float[][] DtCopy = new float[N][];
			for (int i = 0; i < N; ++i) {
				DtCopy[i] = new float[DI];
				for (int j = 0; j < DI; ++j) {
					DtCopy[i][j] = scanner.nextFloat();
				}
			}
			scanner.close();
			
			for (int i = 0; i < M; ++i) {
				Dt[i] = DtCopy[subset[r][i]];
			}
